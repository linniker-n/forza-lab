...
import { adaptTuneToCar, getPresetTune } from "./presets"

const CLASS_PI_MAP: Record<CarClass, number> = {
  D: 500, C: 600, B: 700, A: 800, S1: 900, S2: 950, R: 975, X: 999,
}

function estimatePI(car: Car, targetClass: CarClass): number {
  const base = CLASS_PI_MAP[targetClass]
  return Math.min(base, CLASS_PI_MAP[targetClass])
}
...
function generateWarnings(car: Car, tuneType: TuneType, drivetrain: Drivetrain, requestedDrivetrain: TuneRequest["preferred_drivetrain"]): TuneWarning[] {
  ...
  if (requestedDrivetrain !== "original" && requestedDrivetrain !== drivetrain) {
    warnings.push({
      type: "warning",
      message: `Tração ${requestedDrivetrain} não está marcada como conversão disponível para este carro. A tune foi recalculada usando ${drivetrain}.`,
    })
  }
  ...
  if (car.power_hp > 700 && tuneType === "street") {
    warnings.push({ type: "tip", message: "Muita potência para rua. Ajuste o diferencial para evitar wheelspin em aceleração." })
  }
...
export function generateTune(request: TuneRequest, car: Car): GeneratedTune {
  ...
  const parts = selectParts(car, profile, request.tune_type, drivetrain)
  const basePreset = getPresetTune(request.tune_type)
  const tuning = adaptTuneToCar(basePreset, profile, request.tune_type, drivetrain)

  if (request.difficulty === "easy") {
    tuning.differential.rear_accel = Math.max(30, (tuning.differential.rear_accel ?? 60) - 15)
    tuning.antiroll_bars.front = Math.max(10, tuning.antiroll_bars.front - 4)
    tuning.antiroll_bars.rear = Math.max(10, tuning.antiroll_bars.rear - 4)
  } else if (request.difficulty === "aggressive") {
    tuning.differential.rear_accel = Math.min(100, (tuning.differential.rear_accel ?? 60) + 15)
    tuning.antiroll_bars.front += 4
    tuning.antiroll_bars.rear += 3
  }
  ...
  return {
    ...
    pi_estimate: estimatePI(car, request.target_class),
  }
}
